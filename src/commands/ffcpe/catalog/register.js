...
export default class CatalogRegister extends Command {
  async run () {
    const { flags } = await this.parse(CatalogRegister)
    ...
    const res = await withCatalogSpinner('Registering catalog action', () =>
      catalogFetch({
        baseUrl,
...
CatalogRegister.description = 'Register a new custom catalog action from JSON'

CatalogRegister.examples = [
  '$ aio ffcpe catalog register --file ./catalog-entry.json'
]

CatalogRegister.flags = {
  ...catalogBaseFlags,
  file: Flags.string({
    char: 'f',
